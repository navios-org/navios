...
export function InjectOptional<T>(token: AnyTokenOrClass, args?: unknown) {
  return (
    _target: ClassAccessorDecoratorTarget<unknown, T | null>,
    context: ClassAccessorDecoratorContext<unknown, T | null>,
  ): ClassAccessorDecoratorResult<unknown, T | null> | void => {
    if (context.kind !== 'accessor') {
      throw new Error(
        '[DI] @InjectOptional must be applied to an accessor field (`accessor foo!: Foo | null`).',
      )
    }
    if (!context.metadata) {
      throw new Error(
        '[DI] @InjectOptional requires decorator metadata support (`context.metadata`).',
      )
    }
