...
describe('GET variants', () => {
  ...
  it('should handle parallel requests with isolated request scoped services', async () => {
    ...
    const requests = [
      supertest(server.getServer().server).get('/request-scoped?data=request1'),
      supertest(server.getServer().server).get('/request-scoped?data=request2'),
      supertest(server.getServer().server).get('/request-scoped?data=request3'),
      supertest(server.getServer().server).get('/request-scoped?data=request4'),
      supertest(server.getServer().server).get('/request-scoped?data=request5'),
    ]
