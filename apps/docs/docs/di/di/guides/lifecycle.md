...
### ⚠️ Why Use Lifecycle Hooks?

**Important**: Services can go through initialization twice, which means accessing injected services in constructors can lead to unexpected behavior. Lifecycle hooks like `onServiceInit()` are called after all dependencies are fully resolved and ready to use, making them the safe place for initialization logic.
...
### 1. Never Access Services in Constructors

**Critical**: Do not access injected services in constructors. Use `onServiceInit()` instead.
...
class ProblematicService implements OnServiceInit {
  ...
  constructor() {
    ...
    this.logger.log('Service created') // Can fail or behave unexpectedly
  }

  async onServiceInit() {
    // ✅ Safe: All dependencies are ready here
...
class CorrectService implements OnServiceInit {
  ...
  constructor() {
    // ✅ OK: Only initialize local state, no service access
    this.connection = null
  }

  async onServiceInit() {
    // ✅ All services are ready here
...
- Learn about [request contexts](/docs/di/di/guides/request-contexts) for request-scoped lifecycle
- Explore [services](/docs/di/di/guides/services) for service creation
